...
import { CreatePlanPayload, Plan, UpdatePlanPayload } from "../interfaces/plan.interface";
import { PaginatedResult } from "@/common/interfaces/pagination.interface";

export class PlansService {
    private static readonly ENDPOINT = '/membership-plans';

    static async getAll(page: number = 1, limit: number = 10): Promise<PaginatedResult<Plan>> {
        return await httpClient.get<PaginatedResult<Plan>>(`${this.ENDPOINT}?page=${page}&limit=${limit}`);
    }
