...
export default function MemberDetailPage({
    ...
    return (
      <div className="flex flex-col items-center justify-center h-64 w-full bg-surface border border-danger-main/20 rounded-lg">
        <p className="text-text-main text-sm">
          Error al cargar el perfil o el miembro no existe.
        </p>
        <Link
          href="/dashboard/miembros"
          className="mt-4 text-brand-main hover:underline text-sm"
        >
          Volver a la lista
        </Link>
      </div>
  ...
  if (activeSubscription) {
    ...
    nextDueDate = end.toLocaleDateString('es-ES', {
      year: 'numeric',
      month: 'short',
      day: 'numeric',
    });
  ...
  const safeStatusStyles =
    statusStyles[memberState] || statusStyles['INACTIVE'];
  const defaultAmount = activeSubscription?.plan?.price
            ...
            <div className="bg-surface border border-border-primary rounded-lg p-6 flex flex-col gap-4 transition-colors">
              ...
              <div className="flex flex-col gap-3 mt-1">
                {futureSubscriptions.map((sub) => (
                  <div
                    key={sub.uuid}
                    className="flex justify-between items-center p-3 bg-background border border-border-primary rounded-md"
                  >
                    <div className="flex flex-col gap-1">
                    ...
                    <div className="flex flex-col items-end gap-1">
                      <span className="text-[10px] font-bold px-2 py-0.5 bg-brand-main/10 text-brand-main rounded-sm">
                        EN ESPERA
                      </span>
                      <span className="text-xs text-text-muted">
                        Vence:{' '}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="bg-surface border border-border-primary  rounded-lg p-6 flex flex-col justify-center gap-4 transition-colors">
              ...
              <div className="w-full bg-border-primary h-2 rounded-full overflow-hidden">
                <div
                  className="bg-brand-main h-full rounded-full transition-all duration-500"
