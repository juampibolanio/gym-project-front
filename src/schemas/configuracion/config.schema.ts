...
export const configGeneralSchema = z.object({
  nombreGimnasio: z.string().trim().min(2, "El nombre debe tener al menos 2 caracteres").max(100, "Nombre demasiado largo"),
  telefono: z.string().trim().min(8, "Teléfono inválido").max(20, "Teléfono muy largo"),
  direccion: z.string().trim().min(5, "La dirección es muy corta").max(150, "Dirección demasiado larga"),
});
